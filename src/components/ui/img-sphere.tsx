...
export interface SphereImageGridProps {
  ...
  className?: string;
  aspectRatio?: number; // Added to support wide images
}
...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const calculateWorldPositions = useCallback((): WorldPosition[] => {
    ...
    for (let i = 0; i < adjustedPositions.length; i++) {
      ...
      // Approximate size for collision (using height as base)
      const imageSize = baseImageSize * adjustedScale;
      // Effectively use a larger collision radius to account for aspect ratio
      const collisionSize = imageSize * (1 + (aspectRatio - 1) * 0.5);
      ...
      for (let j = 0; j < adjustedPositions.length; j++) {
        ...
        const otherCollisionSize = otherSize * (1 + (aspectRatio - 1) * 0.5);

        const dx = pos.x - other.x;
        const dy = pos.y - other.y;
        const distance = Math.sqrt(dx * dx + dy * dy);

        const minDistance = (collisionSize + otherCollisionSize) / 2 + 10;
  ...
  }, [imagePositions, rotation, actualSphereRadius, baseImageSize, aspectRatio]);
  ...
  const renderImageNode = useCallback((image: ImageData, index: number) => {
    ...
    const baseH = baseImageSize * position.scale;
    ...
    return (
      ...
      >
        {/* REMOVED: shadow-lg, border, rounded, bg-white for transparent floating effect */}
  ...
  }, [worldPositions, baseImageSize, containerSize, hoveredIndex, aspectRatio]);
  ...
  const renderSpotlightModal = () => {
    ...
    return (
      ...
      >
        ...
        >
          <div className="relative w-full bg-white flex justify-center items-center p-8">
            <img
            ...
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-2 right-2 w-8 h-8 bg-black/10 hover:bg-black/20 rounded-full text-black flex items-center justify-center transition-all cursor-pointer"
            >
              <X size={16} />
            </button>
          </div>
